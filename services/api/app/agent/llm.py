...
def build_general_chat_agent(settings: Settings, tools: Sequence[object]):
    ...
    return create_agent(
        ...
        system_prompt=(
            ...
            "当用户请求搜索或实时信息时，必须直接调用 web_search 工具执行搜索后再回答；"
            "对于训练知识无法确认的内容或涉及时效性信息的问题（新闻、价格、行情、汇率、天气、最新进展等），"
            "也必须调用 web_search 获取最新资料后再回答，禁止仅凭训练知识作答或臆测。"
            "禁止只输出“我来帮你搜索”“好的，马上”之类的承诺性话术而不实际调用工具。"
